use std::io;
fn bruteforce(cur_map: &mut Vec<String>, res: Option<usize>) -> Option<usize>{
    for i in 0..cur_map.len(){
        for j in 0..cur_map[i].len(){
            let char=cur_map[i].chars().nth(j).unwrap();
            if char=='#'{
                cur_map[i].replace_range(j..j+1,".");
            }
            else if char=='.'{
                cur_map[i].replace_range(j..j+1,"#");
            }
            let cur_res=analyze(cur_map.to_vec(), res);
            if cur_res.is_some(){
                return cur_res;
            }
            if char=='#'{
...
fn analyze(cur_map: Vec<String>, res: Option<usize>) -> Option<usize>{
...
fn main() -> io::Result<()> {
    ...
    for _ in 0..line_count{
        ...
        io::stdin().read_line(&mut buffer)?;
        if buffer.len()==1{
            let res = analyze(row_map.clone(), None);
            let res = bruteforce(&mut row_map, res);
            if res.is_some(){
                sum_rows+=res.unwrap();
            }
            let res = analyze(column_map.clone(), None);
            let res = bruteforce(&mut column_map, res);
            if res.is_some(){
