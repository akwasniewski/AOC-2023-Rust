...
fn dijkstra(map: Vec<Vec<i32>>, mut dist: Vec<Vec<[[i32;10];4]>>) -> i32{
    ...
    while !que.is_empty(){
        ...
        if cur.pos.0>=columns || cur.pos.1>=rows || cur.straight>10 || cur.pos.0<0 || cur.pos.1<0 || (cur.dir !=4 && cur.cost > dist[cur.pos.1 as usize][cur.pos.0 as usize][cur.dir as usize][cur.straight as usize -1]){
            continue;
        }
